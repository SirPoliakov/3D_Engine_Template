...
void PhysicsSystem::testSweepAndPrune(std::function<void(class Actor*, class Actor*)> f)
{
	// Sort by min.x
	std::sort(
				begin(boxes), end(boxes), [](BoxComponent* a, BoxComponent* b)
				{
	...
	for (size_t i = 0; i < boxes.size(); i++)
	{
		...
		for (size_t j = i + 1; j < boxes.size(); j++)
		{
			...
			else if (Collisions::intersect(a->getWorldBox(),
				b->getWorldBox()))
			{
